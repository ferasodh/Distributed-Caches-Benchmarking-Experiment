...
LOGS_BASE=logs-$(date +"%Y%m%d-%H%M%S-%N")
...
if [[ "${OUTPUT_FOLDER}" == "" ]] && [[ ${CONFIG} != *'-of '* ]] && [[ ${CONFIG} != *'--outputFolder '* ]]; then
    folder=results-$(date +"%Y%m%d-%H%M%S-%N")
...
do
    ...
    file_log=${LOGS_DIR}"/"${now}"_id"${id}"_driver.log"
    ...
    echo "<"$(date +"%H:%M:%S")"><yardstick> Driver with id=${id} is stopped"
